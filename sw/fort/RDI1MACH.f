...
c ---------------------------------------------------------------------
c  Fortran-90 versions of machine-constant routines R1MACH, D1MACH, I1MA
c
...
c  References:
c  
c  David Gay and Eric Grosse, Comment on Algorithm 528, Bell Labs, Murra
c    Hill, NJ. submitted to ACM Transactions on Mathematical Software,
c    August 1996.
c
c http://www.nsc.liu.se/~boein/ifip/kyoto/workshop-info/proceedings/eina
...
c --------------------------------------------------------------------
      ...
      SELECT CASE (I)
        ...
        CASE (5)
          R1MACH = LOG10(B)
        CASE DEFAULT
      CALL RRTMERR('RDI1MACH.f:107:R1MACH -- input argument out of ',53)
      END SELECT
...
c --------------------------------------------------------------------
      ...
      SELECT CASE (I)
        ...
        CASE (5)
          D1MACH = LOG10(B)
        CASE DEFAULT
      CALL RRTMERR('RDI1MACH.f:170:D1MACH -- input arg out of bound',48)
      END SELECT
...
c                sign (B**E)*( (X(1)/B) + ... + (X(T)/B**T) )
c
c     where 0 <= X(I) .LT. B for I=1,...,T; 0 < X(1); and EMIN <= E <= E
c
c     I1MACH(10) = B, the base.
c
c   Single-Precision:
...
c --------------------------------------------------------------------
      ...
      SELECT CASE (I)
        ...
        CASE (3)
      CALL RRTMERR('RDI1MACH.f:249:I1MACH: input arg = 3 is obsolet',48)
        CASE (4)
          I1MACH = 0 ! Error message unit
        CASE (5)
          I1MACH = BIT_SIZE(I)
        CASE (6)
      CALL RRTMERR('RDI1MACH.f:255:I1MACH: input arg = 6 is obsolet',48)
        CASE (7)
          I1MACH = RADIX(1)
        CASE (8)
          I1MACH = BIT_SIZE(I) - 1
        ...
        CASE DEFAULT
      CALL RRTMERR('RDI1MACH.f:277:I1MACH: input argument out of bo',51)
      END SELECT
