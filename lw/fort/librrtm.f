C     A library-style interface for RRTM.
C
C     Subroutines:
C     ============
C
C     INITRRTM (ISCAT, NUMANGS)
C     INITSURFACE (IEMISS,TBOUND,IREFLECT,SEMIS)
C     INITPROFILE (NLAYERS, TAVEL, PAVEL, TZ, PZ, NMOL, WKL, WBRODL)
C     EXECRUN ()
C


      SUBROUTINE INITRRTM(ISCATINP,NUMANGSINP)

C     Sets the defaults for RRTM.
...
Cf2py intent(in) ISCAT
Cf2py intent(in) NUMANGS

      COMMON /OUTPUT/    TOTUFLUX(0:MXLAY), TOTDFLUX(0:MXLAY),
     $     FNET(0:MXLAY), HTR(0:MXLAY)
      COMMON /RTTBL/     BPADE, TAUTBL(0:NTBL),TRANS(0:NTBL), TF(0:NTBL)

      HEATFAC = 1.0E-7*(GRAV * SECDY)/(CPDAIR)
      ONEMINUS = 1. - 1.E-6
      FLUXFAC = PI * 2.D4  

C     Forced settings - we do not currently support modifying these.
      IATM = 0
      IXSECT = 0
      ICLD = 0
      IOUT = 0
...
C     Some differences between readascii and the functions;
      IXMAX = MAXINPX

C     Process the NUMANGS and ISCAT arguments
...
C
C     A subroutine that should be called after the profile data and
C     surface data has been read in.
C
...
C     computed as a function of the tau transition function,
C     transmittance is calculated as a function of tau, and the tau
C     transition function is calculated using the linear in tau
C     formulation at values of tau above 0.01.  TF is approximated as
C     tau/6 for tau < 0.01.  All tables are computed at intervals of
C     0.001.  The inverse of the constant used in the Pade approximation
C     to the tau transition function is set to b.  These values are not
C     necessary when using DISORT as the RT solver.
...
C     Some setup of the ISTART and IEND parameters
      ISTART = 1
      IEND = 16
      IFLAG = IOUT
      ...
      SUBROUTINE INITSURFACE(IEMISS,TBOUNDINP,IREFLECTINP,SEMIS)
C
C     Initialises the surface emissivity, temperature, etc.
C
      PARAMETER (MXLAY=603)
      PARAMETER (MG = 16)
      PARAMETER (NBANDS = 16)
      ...
      REAL*8 TBOUNDINP
      INTEGER*8 IREFLECTINP
      REAL*8 SEMIS(NBANDS)
      
Cf2py intent(in) iemissinp
Cf2py intent(in) tboundinp
Cf2py intent(in) ireflectinp
Cf2py intent(in) semissinp

      TBOUND = TBOUNDINP
      IREFLECT = IREFLECTINP
 ...
 1500 CONTINUE
      ...
      SUBROUTINE INITPROFILE(NLAYERSINP,TAVELINP,PAVELINP,TZINP,PZINP,
     &     NMOLINP,WKLINP,WBRODLINP)
C
C     Setup the profiles of temperature, pressure and molecular species.
C
      PARAMETER (MXLAY=603, MXMOL=39)
      PARAMETER (MG = 16)
      PARAMETER (NBANDS = 16)
...
C     zero all WKL entries -- we might not fill this array.
      DO 1000 L = 1, NLAYERS
         DO 1500 M = 1, MXMOL
            WKL(M,L) = 0
 1500    CONTINUE
...
C     copy across the input data into the common block variables
      NMOL = 7
      TZ(0) = TZINP(0)
      PZ(0) = PZINP(0)
      DO 2000 L = 1, NLAYERS
...
C     Process the chemical species
      DO 5000 L = 1, NLAYERS
         SUMMOL = 0.0
         DO 4100 IMOL = 2, NMOL
...
C     Most of these possibilities are not available as clouds always off
...
C     Read in cloud input option.  
 ...
 1000 CONTINUE
      ...
      IF (LAY .LE. NPRELAY)
     &CALL RRTMERR('librrtm.f:947:CLD LAYERS NOT IN ASCENDING ORDER',47)
     ...
     &CALL RRTMERR('librrtm.f:956:CLDFRAC MUST BE 1 WHEN USING DISO',49)
...
c
c    Constants from NIST May 2010 (and other constants that will need mo
