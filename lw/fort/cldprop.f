...
C                  cloud optical depth (and if ISCAT=2) single-scatterin
C                  and phase-function moments are input.	 
C     INFLAG = 1:  For each cloudy layer, the cloud fraction and cloud
C                  water path (g/m2) are input.  The (gray) cloud optica
...
C       ICEFLAG = 0:  The ice effective radius (microns) is input and th
C                     optical depths due to ice clouds are computed as i
C       ICEFLAG = 1:  The ice effective radius (microns) is input and th
C                     optical depths due to ice clouds are computed as i
C                     Ebert and Curry, JGR, 97, 3831-3836 (1992).  The 
C                     spectral regions in this work have been matched wi
...
C                    Q. Fu, J. Climate, (1998). Q. Fu provided high reso
...
C                    140.0 micron.
C       LIQFLAG = 0:  The optical depths due to water clouds are compute
C                     in CCM3.
...
C                   cloud optical depth (and if ISCAT=2) single-scatteri
C                   and phase-function moments are input for each band.	
     ...
     &CALL RRTMERR('cldprop.f:146:CLDFRAC MUST BE 1 WHEN USING DISO',49)
               ICLDATM = 1
...
C              Calculation of absorption coefficients due to ice clouds.
      ...
      IF (RADICE .LT. 10.0)
     &CALL RRTMERR('cldprop.f:173:ICE RADIUS TOO SMALL',34)
...
C A CLEAR LAYER SEPARATES CLOUDY LAYERS, RESET COUNTER
 ...
 3000    CONTINUE
     ...
     &CALL RRTMERR('cldprop.f:304:CLDFRAC MUST BE 1 WHEN ISCAT=1,2',46)
               ICLDATM = 1
               
               IF (INFLAG .EQ. 1) THEN
                  PRINT*,'INFLAG OPTION 1 NOT ALLOWED WITH ISCAT=2'
      CALL RRTMERR('cldprop.f:309:',14)
               ENDIF
...
C     Separate treatment of ice clouds and water clouds.
 ...
 5200                CONTINUE
                  ...
                  IF (ICEFLAG .EQ. 3) THEN
                     IF (RADICE .LT. 5.0 .OR. RADICE .GT. 140.)
     &CALL RRTMERR('cldprop.f:354:ICE RADIUS OUT OF BOUNDS',38)
                     NCBANDS = 16
                     FACTOR = (RADICE-2)/3.
                     INDEX = INT(FACTOR)
                     IF (INDEX .EQ. 46) INDEX = 45
...
C     ABSICEn(J,IB) are the parameters needed to compute the liquid wate
